 ...
 * Copyright (c) 2018-2019 ARM Germany GmbH. All rights reserved.
 *------------------------------------------------------------------------------
...
#include "RTE/USB/USBD_Config_CDC_0.h"
...
bool USBD_CDC0_ACM_SendEncapsulatedCommand (const uint8_t *buf, uint16_t len) {
  ...
  if (get_encapsulated_response_len != 0U) {
    ...
    USBD_CDC_ACM_Notify_ResponseAvailable (0);
  }
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if ((link_state  == ARM_ETH_LINK_UP)        &&
        ...
        do {
          usb_cdc_acm_status = USBD_CDC_ACM_WriteData (0, (const uint8_t *)ptr_packet_msg, ptr_packet_msg->MessageLength);
